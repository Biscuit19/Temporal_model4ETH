...
def data_padding(all_trans_list):
	...
	# 处理整个数据集
	for all_windows in tqdm(all_trans_list, desc='Processing DataPadding', unit='data'):
...
def conv_train_data_generate(pkl_file):
	...
	def filter_account():
		...
		for account, attributes in acct_dict.items():
			if attributes['category'] == 1 and count_range[0] <= attributes['all_cnt'] < count_range[1]:
	...
	# 为了对应标签
	sample= [[attribute['all_trans'],attribute['category']] for address, attribute in sample_dict.items()]

	# 分离出trans
	sampled_trans = [item[0] for item in sample]
	# 分离出所有类别标签
	sampled_labels = [item[1] for item in sample]

	labels=torch.tensor(sampled_labels)
