...
class Classifier_ConvLSTM(nn.Module):
	def __init__(self, input_channels, hidden_channels, kernel_size, num_layers, batch_first, bias, return_all_layers):
		super(Classifier_ConvLSTM, self).__init__()
		self.convlstm = ConvLSTM(input_channels, hidden_channels, kernel_size, num_layers, batch_first, bias,
								 return_all_layers)

		self.fc = nn.Linear(hidden_channels, 1)  # 假设hidden_channels是LSTM的隐藏层维度


	def forward(self, x):
		# 使用ConvLSTM得到输出
		lstm_out, _ = self.convlstm(x)
		# lstm_out的形状为[batch_size, seq_len, hidden_dim, height, width]
		# 取最后一个时间步的输出
		lstm_out = lstm_out[0][:, -1, ...]  # 现在的形状为[batch_size, hidden_dim, height, width]
		# 全局平均池化（Global Average Pooling）
		gap = torch.mean(lstm_out, dim=[2, 3])  # 对height和width维度进行平均，形状变为[batch_size, hidden_dim]
		# 通过全连接层
		out = self.fc(gap)
		# 使用sigmoid激活函数，因为是二分类问题
...
def print_label_counts(dataloader, dataset_type):
    label_counts = [0, 0]
    for _, labels in dataloader:
        for label in labels:
            label_counts[label] += 1

    print(f"{dataset_type} dataset label counts:")
    print("Label 0 (Negative samples):", label_counts[0])
    print("Label 1 (Positive samples):", label_counts[1])
...
def model_train():
	...
	data_file = 'conv_train_data_classifer.pkl'
	...
	# 创建 TensorDataset
	dataset = TensorDataset(data_batch, labels)

	# 确定训练集和测试集的大小
	total_size = len(dataset)
	train_size = int(0.8 * total_size)  # 假设训练集占80%
	...
	# 在训练集上调用该函数
	print_label_counts(train_dataloader, "Training")

	# 在测试集上调用该函数
	print_label_counts(test_dataloader, "Testing")
	...
	lr=0.01
	# 将模型移到GPU上
	Classifier_ConvLSTM_model = Classifier_ConvLSTM(input_dim, hidden_dim, kernel_size, num_layers, bias, batch_first,
													return_all_layers).to(device)
	print(f'(hidden_dim,kernel_size,num_layers,lr):{hidden_dim,kernel_size,num_layers,lr}')

	criterion = nn.BCELoss()
	criterion.to(device)
	optimizer = optim.Adam(Classifier_ConvLSTM_model.parameters(), lr=lr)
	...
	for epoch in range(epoch_num):
		Classifier_ConvLSTM_model.train()  # 切换到训练模式

		total_loss = 0.0
		for batch_data, labels in train_dataloader:  # 确保DataLoader返回数据和标签
			batch_data = batch_data.to(device)
			labels = labels.to(device).float()  # 确保标签也是浮点类型
			optimizer.zero_grad()
			outputs = Classifier_ConvLSTM_model(batch_data)
			loss = criterion(outputs.squeeze(), labels)  # 对输出应用squeeze()以匹配labels的形状
			loss.backward()
			optimizer.step()
			total_loss += loss.item()

		Classifier_ConvLSTM_model.eval()  # 切换到评估模式
		all_outputs = []
		all_labels = []
		with torch.no_grad():
			for val_batch_data, val_labels in test_dataloader:
				val_batch_data = val_batch_data.to(device)
				val_labels = val_labels.to(device).float()

				outputs = Classifier_ConvLSTM_model(val_batch_data).squeeze()
				predictions = (outputs > 0.5).float()  # 将输出概率转换为二分类预测
		...
		print(
			f'Epoch [{epoch + 1}/{epoch_num}], Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}, FPR: {fpr:.4f}, Accuracy: {accuracy:.4f}')

	# 保存模型的权重和结构
	model_path = './Classifier_ConvLSTM_model.pth'  # 指定保存的文件路径
	torch.save(Classifier_ConvLSTM_model.state_dict(), model_path)
...
def test():
	x = torch.rand((100, 31, 4, 300, 1))

	# (batch_size, sequence_length, Channels, Height, Width)
	# 创建自定义的ConvLSTM网络
	AECL_model = Classifier_ConvLSTM(input_channels=4, hidden_channels=64, kernel_size=(301, 1), num_layers=1,
									 batch_first=True,
	...
	output, h_c = AECL_model.forward(x)
